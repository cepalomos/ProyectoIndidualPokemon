...
async function getDbPokemon() {
  try {
    ...
    pokemons = pokemons.map(({ id, name, attack, types }) => ({
      id,
      name,
      attack,
      types: types.map((typo) => typo.name),
    }));
    return pokemons;
  } catch (error) {
    throw new Error(error);
...
async function savePokemonDb(data) {
  ...
  try {
    const verificar = await Pokemon.findOne({
      where: { name: pokemonData.name.toLowerCase() },
    });
    if (!verificar){
      await Pokemon.create(pokemonData).then((pokemon) =>
        pokemon.addType(typeData)
      );
    return true;}
